(defun android-generate-keystore ()
  ...
  (defun get-passwords (count)
    ...
    (cond
     ...
     ((string-equal
        firstPassword
        repeatedPassword) (cons firstPassword (concat
                                                firstPassword    "\n"
                                                repeatedPassword "\n")))
  ...
  (defun get-rest (a p)
    ...
    (if (yes-or-no-p (concat "Is CN=" name  ", OU="  orgUnit
                             ", O="   org   ", L="   city
                             ", ST="  state ", C="   twoLetter " correct? "))
        (concat name  "\n" orgUnit   "\n"
                org   "\n" city      "\n"
                state "\n" twoLetter "\nyes\n"
                (read-string
  ...
  (setq filePath   (read-directory-name "Directory to create keystore in: ")
        ...
        keytoolCmd (concat
                     ...
                     alias
                     " -keyalg RSA -keysize "
                     (read-string "Keystore size (\"2048\"): " nil nil "2048")
                     " -validity "
                     (read-string "Keystore validity (\"10000\"): " nil
                                  nil                               "10000")
  ...
  (if passwords
      (progn
        (setq command (concat "cd "         filePath
                              "; printf \"" (cdr passwords)
